...
angular.module('cdiService')
    ...
    controller: ['$scope', '$interval', '$timeout', '$sce', 'CdiWidgetService', 'CDI_CONFIG',
        function ($scope, $interval, $timeout, $sce, CdiWidgetService, CDI_CONFIG) {
            ...
            var previousBarStatus = {};
            // Previous lines and inputs to detect changes
            var previousLinesData = {};
            var previousInputsData = {};
            ...
            function loadStatusData() {
                ...
                CdiWidgetService.getLinesStatus(vm.apiDomain)
                    .then(function (data) {
                    var lines = data['LINEAS'] || []
                        .filter(function (line) { return line.status !== 0; });
                    var inputs = data['ENTRADAS'] || []
                        .filter(function (input) { return input.status !== 0; });
            ...
            function updateStatusBarIcons() {
                ...
                var battery = vm.barStatus.battery;
                var batteryIcon = 'batteryfault';
                var batteryAlt = 'Batería: Falla';
                if (battery === 100) {
                ...
                var powerIcon = vm.barStatus.powerSupply ? 'powersupplynormal' : 'powersupplyfault';
                var powerAlt = vm.barStatus.powerSupply ? 'Alimentación OK' : 'Falla de alimentación';
                var networkIcon = vm.barStatus.network ? 'networknormal' : 'networkfault';
                var networkAlt = vm.barStatus.network ? 'Red conectada' : 'Red desconectada';
                vm.statusBarIconsRight = [
            ...
            vm.t = function (path) {
                var keys = path.split('.');
                var value = CDI_CONFIG.DICTIONARY;
            ...
            function orderLines(lines) {
                var customOrder = { 2: 0, 3: 1, 4: 2, 6: 3, 7: 4, 8: 5, 0: 6 };
                return angular.copy(lines).sort(function (a, b) {
                    var _a, _b;
                    return ((_a = customOrder[a.status]) !== null && _a !== void 0 ? _a : 99) - ((_b = customOrder[b.status]) !== null && _b !== void 0 ? _b : 99);
                });
            }
            function orderInputs(inputs) {
                var customOrder = { 12: 0, 9: 1, 1: 2, 4: 3, 5: 4, 0: 5 };
...
//# sourceMappingURL=cdi-widget.component.js.map